...
def get_parameter_number(model):
    ...
    return {
        ...
        "Percent": trainable_num / total_num,
    }
...
def adjust_learning_rate(accelerator, optimizer, scheduler, epoch, args, printout=True):
    if args.lradj == "type1":
        ...
        if epoch > args.least_epochs:
            lr_adjust = {
                epoch: args.learning_rate * (0.5 ** ((epoch - args.least_epochs) // 1))
            }
        else:
...
def vali_baseline(
    ...
    with torch.no_grad():
        for i, (
            ...
            # self.accelerator.wait_for_everyone()
            std, mean_value = (
                np.sqrt(vali_data.label_scaler.var_[-1]),
                vali_data.label_scaler.mean_[-1],
            )
            transformed_preds = outputs * std + mean_value
            transformed_labels = labels * std + mean_value

            all_predictions, all_targets, seen_unseen_ids = (
                accelerator.gather_for_metrics(
                    (transformed_preds, transformed_labels, seen_unseen_ids)
                )
            )
    ...
    if compute_seen_unseen:
        ...
        seen_references = (
            total_references[total_seen_unseen_ids == 1]
            if np.any(total_seen_unseen_ids == 1)
            else np.array([0])
        )
        unseen_references = (
            total_references[total_seen_unseen_ids == 0]
            if np.any(total_seen_unseen_ids == 0)
            else np.array([0])
        )
        seen_preds = (
            total_preds[total_seen_unseen_ids == 1]
            if np.any(total_seen_unseen_ids == 1)
            else np.array([1])
        )
        unseen_preds = (
            total_preds[total_seen_unseen_ids == 0]
            if np.any(total_seen_unseen_ids == 0)
            else np.array([1])
        )
