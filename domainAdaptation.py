...
parser.add_argument(
    "--target_dataset", type=str, default="None", help="the target domain dataset"
)
parser.add_argument(
...
parser.add_argument(
    "--alpha1", type=float, default=0.15, help="the alpha for alpha-accuracy"
)
parser.add_argument(
    "--alpha2", type=float, default=0.1, help="the alpha for alpha-accuracy"
)
...
for ii in range(args.itr):
    ...
    train_data, train_loader, train_target_data, train_target_loader = (
        data_provider_func(
            args,
            "train",
            None,
        ...
        for i in range(len(train_loader)):
            with accelerator.accumulate(model):
                (
                    ...
                    weights,
                    seen_unseen_ids,
                ) = next(iterator_source)
                (
                    target_cycle_curve_data,
                    target_curve_attn_mask,
                    target_labels,
                    target_life_class,
                    target_scaled_life_class,
                    target_weights,
                    target_seen_unseen_ids,
                ...
                outputs, embeddings = model(
                    cycle_curve_data, curve_attn_mask, return_embedding=True
                )
                target_outputs, target_embeddings = model(
                    target_cycle_curve_data,
                    target_curve_attn_mask,
