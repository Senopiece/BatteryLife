...
def calculate_metrics_based_on_seen_number_of_cycles(
    total_preds,
    total_references,
    ...
    with open(
        f"{output_path}number_MAPE_{model}_{dataset}_{finetune_dataset}_{seed}.json",
        "w",
    ) as f:
        json.dump(number_MAPE, f)
    with open(
        f"{output_path}number_alphaAcc1_{model}_{dataset}_{finetune_dataset}_{seed}.json",
        "w",
    ) as f:
        json.dump(number_alphaAcc1, f)
    with open(
        f"{output_path}number_alphaAcc2_{model}_{dataset}_{finetune_dataset}_{seed}.json",
        "w",
    ) as f:
...
parser.add_argument(
    "--data", type=str, required=False, default="Dataset_original", help="dataset type"
)
parser.add_argument(
    "--root_path",
    type=str,
    default="./dataset/HUST_dataset/",
...
# forecasting task
parser.add_argument(
    "--early_cycle_threshold", type=int, default=100, help="what is early life"
)
...
parser.add_argument(
    "--eval_batch_size", type=int, default=16, help="batch size of model evaluation"
)
...
parser.add_argument(
    "--use_LoRA", action="store_true", default=False, help="Set True to use LoRA"
)
parser.add_argument("--LoRA_r", type=int, default=8, help="r for LoRA")
parser.add_argument(
    "--LoRA_dropOut", type=float, default=0.1, help="dropout rate for LoRA"
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="the True to save the results; Only effective when eval_cycle_min or eval_cycle_max is smaller than 0",
)
parser.add_argument(
...
parser.add_argument(
    "--args_path", type=str, help="the path to the pretrained model parameters"
)
parser.add_argument("--eval_dataset", type=str, help="the target dataset")
parser.add_argument(
    "--eval_cycle_min", type=int, default=10, help="The lower bound for evaluation"
)
parser.add_argument(
    "--eval_cycle_max", type=int, default=10, help="The upper bound for evaluation"
)
...
for ii in range(args.itr):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        "deepset/sentence_bert", trust_remote_code=True
    ...
    test_loader, model, model_optim = accelerator.prepare(
        test_loader, model, model_optim
    )
    ...
    with torch.no_grad():
        for i, (
            ...
            cycle_curve_data = cycle_curve_data.float().to(
                accelerator.device
            )  # [B, S, N]
            ...
            total_dataset_ids = (
                total_dataset_ids
                + dataset_ids.detach().cpu().numpy().reshape(-1).tolist()
            )
            total_seen_unseen_ids = (
                total_seen_unseen_ids
                + seen_unseen_ids.detach().cpu().numpy().reshape(-1).tolist()
            )
            total_seen_number_of_cycles = (
                total_seen_number_of_cycles
                + seen_number_of_cycles.detach().cpu().numpy().reshape(-1).tolist()
            )
    ...
    if accelerator.check_trigger():
        ...
        accelerator.print(
            f"Eval cycle: {eval_cycle_min}-{eval_cycle_max} | MAPE: {mape} | {alpha}-accuracy: {alpha_acc}% | {alpha2}-accuracy: {alpha_acc2}%"
        )
        ...
        if len(unseen_references) == 0:
            accelerator.print(
                f"Eval cycle: {eval_cycle_min}-{eval_cycle_max} | Seen MAPE: {seen_mape} | Seen {alpha}-accuracy: {seen_alpha_acc}%"
            )
        else:
            unseen_mape = mean_absolute_percentage_error(
                unseen_references, unseen_preds
            )
            relative_error = abs(unseen_preds - unseen_references) / unseen_references
            hit_num = sum(relative_error <= alpha)
            unseen_alpha_acc = hit_num / len(unseen_references) * 100
            accelerator.print(
                f"Eval cycle: {eval_cycle_min}-{eval_cycle_max} | Seen MAPE: {seen_mape} | Unseen MAPE: {unseen_mape} | Seen {alpha}-accuracy: {seen_alpha_acc}% | Unseen {alpha}-accuracy: {unseen_alpha_acc}%"
            )
        ...
        if len(unseen_references) == 0:
            ...
            accelerator.print(
                f"Eval cycle: {eval_cycle_min}-{eval_cycle_max} | Seen {alpha2}-accuracy: {seen_alpha_acc2}%"
            )
            ...
            accelerator.print(
                f"Eval cycle: {eval_cycle_min}-{eval_cycle_max} | Seen {alpha2}-accuracy: {seen_alpha_acc2}% | Unseen {alpha2}-accuracy: {unseen_alpha_acc2}%"
            )

        if eval_cycle_min is None or eval_cycle_max is None:
            calculate_metrics_based_on_seen_number_of_cycles(
                total_preds,
                total_references,
                total_seen_number_of_cycles,
                alpha,
                ...
                start=args.seq_len,
                end=args.early_cycle_threshold,
                seed=args.seed,
            )
